...
with open(os.path.join(OUTPUT_FILE,'sc_model_x.pkl'),'rb') as f:
    sc_model_x=pickle.load(f)
with open(os.path.join(OUTPUT_FILE,'sc_model_y.pkl'),'rb') as f:
    sc_model_y=pickle.load(f)

with open(os.path.join(OUTPUT_FILE,'sc_model.pkl'),'rb') as f:
  model=pickle.load(f)

#推測結果を出力
print('Step5. 推測結果を出力')
x=df.iloc[:,2:]
sc_x=sc_model_x.transform(x)    #標準化
sc_y_pred=model.predict(sc_x)
y_pred=sc_model_y.inverse_transform(sc_y_pred) #標準化前に逆変換
